...
#        self.pmw_rw.frequency = 0
        ...
        self.servo_input = 1 #boolean for input servo function
...
#        self.pwm_rw.frequency = 0
    ...
    def servo_turn_to_angle_2(self,i,n):#i is channel and n is angle
#        print("n:",n)
#        print("s0 pos:",self.servo0_pos)
#        print("s1 pos:",self.servo1_pos)
        if i==0 and n<=180 and n>=0:#restrict motion to 180 degrees
            if n>self.servo0_pos:#if servo needs to move anti-clockwise
#                print("in first loop")
                for j in range(self.servo0_pos,n):
#                    print(j)
                    self.servo0.angle = j
            else:#if servo moves clockwise
...
#                    print(j)
    ...
    def input_servo_value(self):
        self.servo_turn(0,0)
        time.sleep(1)
        self.servo_turn(0,0)
        time.sleep(1)
        print("Calibration complete")
        while self.servo_input ==1:
            print("in input function")
            i, n = input("Please enter two numbers comma separated (i,n):").split(',')
            i=int(i)
            n=int(n)
            if i== 0 or i == 1 and n >= 0 and n <= 180:
#                self.servo_turn(i,n)
                self.servo_turn_to_angle_2(i,n)
            else:
                print("An unknown input was entered. Please try again.")
                self.servo_input =0
        self.servo_terminate()

    def servo_turn(self,i,n):
        if i==0:
            self.servo0.angle=n
            self.servo0_pos=n
        elif i==1:
